...
# Replacing text formatting with xml tags.
...
outdata = File.read(new_file).gsub(/ER\s\s-\s/, "#{replace_ER}").gsub(/\tER/, "#{replace_ER}").gsub(/ER\r/, "#{replace_ER}").gsub(/\t<\//, "#{remove_end_space}").gsub(/ <\//, "#{remove_end_space}")
File.open(new_file, 'w') do |out|
  out << outdata
end  

puts "Formatted text has been converted to solr document tags."
